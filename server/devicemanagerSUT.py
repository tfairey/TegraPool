...
class DeviceManagerSUT(DeviceManager):
  ...
  def adjustResolution(self, width=1680, height=1050, type='hdmi'):
    ...
    return True
  """
    Switches the adb bridge to use the given transportation method (usb or ip)
  """
  def adjustADBTransport(self, transport='ip'):
    if transport != 'usb' and transport != 'ip':
      return None
    try:
      data = self.verifySendCMD(['adb ' + transport])
    except(DMError):
      return None
    return data;
